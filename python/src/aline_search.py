...
def _parse_datetime(dt_str):
    if not dt_str:
        return None
    return datetime.strptime(dt_str, "%Y年%m月%d日 %H:%M").strftime("%Y-%m-%d %H:%M:%S")


def fetch_results():
    today = date.today().strftime("%Y年%m月%d日")
    search_conditions = [
        {"startDate": today, "startPort": "78", "endPort": "83"},
        {"startDate": today, "startPort": "78", "endPort": "50"},
    ]
    results = []
    ...
    with get_connection() as connection:
        with connection.cursor(buffered=True) as cursor:
            ...
            for result in results:
                logging.info(result)
                direction = "上り" if result.get("下船港") == "鹿児島" else "下り"
